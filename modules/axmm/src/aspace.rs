...
impl AddrSpace {
    ...
    pub fn unmap(&mut self, start: VirtAddr, size: usize) -> AxResult {
        let mut page_size = size;
        if let Some(area) = self.areas.find(start) {
            let backend = area.backend();
            if let Backend::Alloc { populate: _, align } = *backend {
                page_size = memory_addr::align_up(size, align.into());
                self.validate_region(start, page_size, align)?;
            } else {
        ...
        self.areas
            .unmap(start, page_size, &mut self.pt)
            .map_err(mapping_err_to_ax_err)?;
        Ok(())
    }
    ...
    pub fn unmap_user_areas(&mut self) -> AxResult {
        for area in self.areas.iter() {
            let backend = area.backend();
            if let Backend::Alloc { populate: _, align } = *backend {
                assert!(area.start().is_aligned(align));
                assert!(area.size() % (align as usize) == 0);
            } else {
