...
//! The design is inspired by the Iterator Wrapper pattern [3],
...
///
/// Wraps the `PageIter` struct with a fixed page size of `PAGE_SIZE_2M`[3]
...
///
/// Wraps the `PageIter` struct with a fixed page size of `PAGE_SIZE_1G`[3]
...
/// access to address iteration [3].
/// The design follows the Iterator Wrapper pattern [3], eliminating type differences
...
/// providing a transparent external access interface [3].
/// The implementation follows the paginated iterator design pattern [3],
...
impl Iterator for PageIterWrapper {
    ...
    /// The original behavior of each page size iterator is preserved [3].
